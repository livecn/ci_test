...
//        $this->CI->config->load('aauth');
    ...
    public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
        ...
        if ($this->config_vars['login_control']['ddos_protection'] && !$this->update_login_attempts()) {

            $this->error($this->CI->lang->line('aauth_error_login_attempts_exceeded'));
            return FALSE;
        }
        if ($this->config_vars['login_control']['ddos_protection'] && $this->config_vars['login_control']['recaptcha_active'] && $this->get_login_attempts() > $this->config_vars['login_control']['recaptcha_login_attempts']) {
            $this->CI->load->helper('recaptchalib');
            $reCaptcha = new ReCaptcha($this->config_vars['login_control']['recaptcha_secret']);
        ...
        if ($this->config_vars['login_control']['login_with_name'] == TRUE) {

            if (!$identifier OR strlen($pass) < $this->config_vars['validation']['password']['min'] OR strlen($pass) > $this->config_vars['validation']['password']['max']) {
            ...
            if (!valid_email($identifier) OR strlen($pass) < $this->config_vars['validation']['password']['min'] OR strlen($pass) > $this->config_vars['validation']['password']['max']) {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['users']);
        ...
        if ($this->config_vars['login_control']['totp_active'] == TRUE AND $this->config_vars['login_control']['totp_only_on_ip_change'] == FALSE AND $this->config_vars['login_control']['totp_two_step_login_active'] == FALSE) {
            if ($this->config_vars['login_control']['totp_two_step_login_active'] == TRUE) {
        ...
        if ($this->config_vars['login_control']['totp_active'] == TRUE AND $this->config_vars['login_control']['totp_only_on_ip_change'] == TRUE) {
            $query = null;
            $query = $this->aauth_db->where($db_identifier, $identifier);
            $query = $this->aauth_db->get($this->config_vars['table']['users']);
            $totp_secret = $query->row()->totp_secret;
            ...
            if ($query->num_rows() > 0 AND ! $totp_code) {
                if ($ip_address != $current_ip_address) {
                    if ($this->config_vars['login_control']['totp_two_step_login_active'] == FALSE) {
                        $this->error($this->CI->lang->line('aauth_error_totp_code_required'));
                        return FALSE;
                    } else if ($this->config_vars['login_control']['totp_two_step_login_active'] == TRUE) {
                        $this->CI->session->set_userdata('totp_required', true);
                    }
                }
            } else {
        ...
        $password = ($this->config_vars['password']['use_password_hash'] ? $pass : $this->hash_password($pass, $row->id));
        ...
        if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) {
            ...
            if ($remember) {
                ...
                $expire = $this->config_vars['login_control']['remember'];
                $today = date("Y-m-d");
            ...
            if ($this->config_vars['login_control']['remove_successful_attempts'] == TRUE) {
    ...
    public function current_user() {
        return $this->get_user($this->CI->session->userdata('id'));
    }
     ...
     */
    public function auth($perm_par) {

        $this->CI->load->helper('url');

        if ($this->CI->session->userdata('totp_required')) {
            $this->error($this->CI->lang->line('aauth_error_totp_verification_required'));
            redirect($this->config_vars['login_control']['totp_two_step_login_redirect']);
        }

        $perm_id = $this->get_perm_id($perm_par);
        $type = $this->get_perm_type($perm_par);

        $this->update_activity();
        if (!$this->is_allowed($perm_id) && !$this->is_group_allowed($perm_id)) {
            if ($type == 'html') {
                ...
                echo json_encode(array('status' => 'fail', 'data' => array(), 'msg' => $this->CI->lang->line('aauth_error_no_access')));
                die();
            }
        }
    }
    ...
    public function reset_login_attempts() {
        ...
        $this->aauth_db->where(
                array(
                    ...
                    'timestamp >=' => date("Y-m-d H:i:s", strtotime("-" . $this->config_vars['login_control']['max_login_attempt_time_period']))
                )
        );
        return $this->aauth_db->delete($this->config_vars['table']['login_attempts']);
    }
    ...
    public function remind_password($email) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->aauth_db->update($this->config_vars['table']['users'], $data);
            ...
            $this->CI->email->message($this->CI->lang->line('aauth_email_reset_text') . site_url() . $this->config_vars['password']['reset_password_link'] . $ver_code);
            $this->CI->email->send();
    ...
    public function reset_password($ver_code) {
        ...
        $this->CI->load->helper('string');
        $pass_length = ($this->config_vars['validation']['password']['min'] & 1 ? $this->config_vars['validation']['password']['min'] + 1 : $this->config_vars['validation']['password']['min']);
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($this->config_vars['login_control']['totp_active'] == TRUE AND $this->config_vars['login_control']['totp_reset_over_reset_password'] == TRUE) {
    ...
    public function update_last_login($user_id = FALSE) {
        ...
        return $this->aauth_db->update($this->config_vars['table']['users'], $data);
    }
    ...
    public function update_login_attempts() {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['login_attempts']);
        ...
        if ($query->num_rows() == 0) {
            ...
            $this->aauth_db->insert($this->config_vars['table']['login_attempts'], $data);
            return TRUE;
        } else {
            $row = $query->row();
            $data = array();
            ...
            $this->aauth_db->where('id', $row->id);
            $this->aauth_db->update($this->config_vars['table']['login_attempts'], $data);

            if ($data['login_attempts'] > $this->config_vars['login_control']['login_with_name']) {
                return FALSE;
            } else {
                return TRUE;
            }
    ...
    public function create_user($email, $pass, $username = FALSE, $active = 1) {
        ...
        if (strlen($pass) < $this->config_vars['validation']['password']['min'] OR strlen($pass) > $this->config_vars['validation']['password']['max']) {
            $this->error($this->CI->lang->line('aauth_error_password_invalid'));
            $valid = FALSE;
        }
        if ($username != FALSE && !ctype_alnum(str_replace($this->config_vars['validation']['additional_valid_chars'], '', $username))) {
        ...
        if ($this->aauth_db->insert($this->config_vars['table']['users'], $data)) {

            $user_id = $this->aauth_db->insert_id();

            // set default group
            $this->add_member($user_id, $this->config_vars['group']['default_group']);

            // if verification activated
            if ($this->config_vars['login_control']['verification'] && !$this->is_admin()) {
                $data = null;
                $data['banned'] = 1;
            ...
            if (!$this->config_vars['password']['use_password_hash']) {
                $data = null;
     ...
     * @return bool Update fails/succeeds
     */
    public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE, $active = 1) {
        ...
        if ($username != FALSE) {
            if ($this->user_exist_by_username($username, $user_id)) {
                $this->error($this->CI->lang->line('aauth_error_update_username_exists'));
                $valid = FALSE;
            }
            if ($username != '' && !ctype_alnum(str_replace($this->config_vars['validation']['additional_valid_chars'], '', $username))) {
                $this->error($this->CI->lang->line('aauth_error_username_invalid'));
                $valid = FALSE;
            }
    ...
    public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE, $sort = FALSE) {
        ...
        if ($group_par != FALSE) {
            ...
            $this->aauth_db->select('*')
                    ->from($this->config_vars['table']['users'])
                    ->join($this->config_vars['table']['user_to_group'], $this->config_vars['table']['users'] . ".id = " . $this->config_vars['table']['user_to_group'] . ".user_id")
                    ->where($this->config_vars['table']['user_to_group'] . ".group_id", $group_par);
    ...
    public function send_verification($user_id) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->CI->email->message($this->CI->lang->line('aauth_email_verification_code') . $ver_code .
                    $this->CI->lang->line('aauth_email_verification_text') . site_url() . $this->config_vars['login_control']['verification_link'] . $user_id . '/' . $ver_code);
            $this->CI->email->send();
        }
    }
    ...
    public function delete_user($user_id) {
        ...
        $this->aauth_db->delete($this->config_vars['table']['perm_to_user']);

        // delete from user_to_group
        $this->aauth_db->where('user_id', $user_id);
        $this->aauth_db->delete($this->config_vars['table']['user_to_group']);

        // delete user vars
        $this->aauth_db->where('user_id', $user_id);
        $this->aauth_db->delete($this->config_vars['table']['user_variables']);

        // delete user
        $this->aauth_db->where('id', $user_id);
        return $this->aauth_db->delete($this->config_vars['table']['users']);
    }
     ...
     * @return bool
     */
    public function user_exist_by_username($name, $id = false) {
     ...
     * @return bool
     */
    public function user_exist_by_email($user_email, $id = false) {
    ...
    public function get_user_groups($user_id = FALSE) {
        ...
        if (!$user_id) {
            $this->aauth_db->where('name', $this->config_vars['group']['public_group']);
            $query = $this->aauth_db->get($this->config_vars['table']['groups']);
        } else if ($user_id) {
            $this->aauth_db->join($this->config_vars['table']['groups'], "id = group_id");
            $this->aauth_db->where('user_id', $user_id);
            $query = $this->aauth_db->get($this->config_vars['table']['user_to_group']);
        }
        return $query->result();
    }
     ...
     * @return array Groups
     */
//            $group_query = $this->db->select('*')->get('aauth_user_to_group');
//            $groups = array();
//            if ($group_query) {
//                if ($groups_array = $group_query->result_array()) {
//                    foreach ($groups_array as $group) {
//                        $groups[$group['user_id']][] = $group['group_id'];
//                    }
//                }
//            }
    public function get_user_group_ids($user_id = FALSE) {
        ...
        if (count($array = $query->result_array())) {
    ...
    public function get_group_user_ids($group_id) {
        if ($group_id) {
            $this->aauth_db->join($this->config_vars['table']['users'], "id = user_id");
    ...
    function hash_password($pass, $userid) {
        if ($this->config_vars['password']['use_password_hash']) {
            return password_hash($pass, $this->config_vars['password']['password_hash_algo'], $this->config_vars['password']['password_hash_options']);
        } else {
            $salt = md5($userid);
            return hash($this->config_vars['password']['hash'], $salt . $pass);
        }
    }
    ...
    public function create_group($group_name, $definition = '') {
        ...
        $query = $this->aauth_db->get_where($this->config_vars['table']['groups'], array('name' => $group_name));
        ...
        if ($query->num_rows() < 1) {
            ...
            $this->aauth_db->insert($this->config_vars['table']['groups'], $data);
    ...
    public function update_group($group_par, $group_name = FALSE, $definition = FALSE) {
        ...
        return $this->aauth_db->update($this->config_vars['table']['groups'], $data);
    }
    ...
    public function delete_group($group_par) {
        ...
        $this->aauth_db->where('group_id', $group_id);
        $this->aauth_db->delete($this->config_vars['table']['perm_to_group']);

        $this->aauth_db->where('group_id', $group_id);
        $this->aauth_db->delete($this->config_vars['table']['group_to_group']);

        $this->aauth_db->where('subgroup_id', $group_id);
        $this->aauth_db->delete($this->config_vars['table']['group_to_group']);

        $this->aauth_db->where('id', $group_id);
        return $this->aauth_db->delete($this->config_vars['table']['groups']);
    }
    ...
    public function add_member($user_id, $group_par) {
        ...
        if ($query->num_rows() < 1) {
            ...
            return $this->aauth_db->insert($this->config_vars['table']['user_to_group'], $data);
        }
    ...
    public function remove_member($user_id, $group_par) {
        ...
        return $this->aauth_db->delete($this->config_vars['table']['user_to_group']);
    }
    ...
    public function add_subgroup($group_par, $subgroup_par) {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['group_to_group']);
        ...
        if ($query->num_rows() < 1) {
            ...
            return $this->aauth_db->insert($this->config_vars['table']['group_to_group'], $data);
        }
    ...
    public function remove_subgroup($group_par, $subgroup_par) {
        ...
        return $this->aauth_db->delete($this->config_vars['table']['group_to_group']);
    }
    ...
    public function is_admin($user_id = FALSE) {
        ...
        return $this->is_member($this->config_vars['group']['admin_group'], $user_id);
    }
    ...
    public function create_perm($perm_name, $definition = '', $type = 'html') {

        $query = $this->aauth_db->get_where($this->config_vars['table']['perms'], array('name' => $perm_name));
        ...
        if ($query->num_rows() < 1) {
            ...
            $this->aauth_db->insert($this->config_vars['table']['perms'], $data);
            return $this->aauth_db->insert_id();
        }
     ...
     * @return bool Update success/failure
     */
    public function update_perm($perm_par, $perm_name = FALSE, $definition = FALSE, $type = FALSE) {
        ...
        return $this->aauth_db->update($this->config_vars['table']['perms'], $data);
    }
    ...
    public function delete_perm($perm_par) {
        ...
        $this->aauth_db->where('id', $perm_id);
        return $this->aauth_db->delete($this->config_vars['table']['perms']);
    }
    ...
    public function is_allowed($perm_par, $user_id = FALSE) {
        ...
        $query = $this->aauth_db->where('user_id', $user_id);
        $query = $this->aauth_db->get($this->config_vars['table']['perm_to_user']);
    ...
    public function is_group_allowed($perm_par, $group_par = FALSE) {
        ...
        if ($group_par != FALSE) {
            ...
            $query = $this->aauth_db->get($this->config_vars['table']['perm_to_group']);
        ...
        // checks current user's all groups
            ...
            if ($this->is_admin($this->CI->session->userdata('id')) OR
                    $this->is_group_allowed($perm_id, $this->config_vars['group']['public_group'])) {
                return TRUE;
            }
    ...
    public function allow_user($user_id, $perm_par) {
        ...
        if ($query->num_rows() < 1) {
            ...
            return $this->aauth_db->insert($this->config_vars['table']['perm_to_user'], $data);
        }
        return TRUE;
    }
     ...
     * @return bool Allow success/failure
     */
    public function user_to_groups($user_id, $group_ids = array()) {
        $this->aauth_db->delete($this->config_vars['table']['user_to_group'], array('user_id' => $user_id));
        if (is_array($group_ids) && count($group_ids)) {
            $data = array();
            foreach ($group_ids as $id) {
                $data[] = array('user_id' => $user_id, 'group_id' => $id);
            }
            return $this->aauth_db->insert_batch($this->config_vars['table']['user_to_group'], $data);
        }
        return TRUE;
    }
     ...
     * @return bool Allow success/failure
     */
    public function group_to_users($group_id, $user_ids = array()) {
        $this->aauth_db->delete($this->config_vars['table']['user_to_group'], array('group_id' => $group_id));
        if (is_array($user_ids) && count($user_ids)) {
            $data = array();
            foreach ($user_ids as $id) {
                $data[] = array('group_id' => $group_id, 'user_id' => $id);
            }
            return $this->aauth_db->insert_batch($this->config_vars['table']['user_to_group'], $data);
        }
        return TRUE;
     ...
     * @return bool Allow success/failure
     */
    public function group_to_perms($group_id, $perm_ids = array()) {
        $this->aauth_db->delete($this->config_vars['table']['perm_to_group'], array('group_id' => $group_id));
        if (is_array($perm_ids) && count($perm_ids)) {
            $data = array();
            foreach ($perm_ids as $id) {
                $data[] = array('group_id' => $group_id, 'perm_id' => $id);
            }
            return $this->aauth_db->insert_batch($this->config_vars['table']['perm_to_group'], $data);
        }
        return TRUE;
    }
     ...
     * @return bool Allow success/failure
     */
    public function user_to_perms($user_id, $perm_ids = array()) {
        $this->aauth_db->delete($this->config_vars['table']['perm_to_user'], array('user_id' => $user_id));
        if (is_array($perm_ids) && count($perm_ids)) {
            $data = array();
            foreach ($perm_ids as $id) {
                $data[] = array('user_id' => $user_id, 'perm_id' => $id);
            }
            return $this->aauth_db->insert_batch($this->config_vars['table']['perm_to_user'], $data);
        }
        return TRUE;
    }
    ...
    public function deny_user($user_id, $perm_par) {
        ...
        return $this->aauth_db->delete($this->config_vars['table']['perm_to_user']);
    }
    ...
    public function allow_group($group_par, $perm_par) {
        ...
        if ($query->num_rows() < 1) {
            ...
            return $this->aauth_db->insert($this->config_vars['table']['perm_to_group'], $data);
        }
    ...
    public function deny_group($group_par, $perm_par) {
        ...
        return $this->aauth_db->delete($this->config_vars['table']['perm_to_group']);
    }
    ...
    public function list_perms() {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['perms']);
        return $query->result_array();
    }
     ...
     * @return int Permission id or NULL if perm does not exist
     */
    public function get_perm_type($perm_par) {
    ...
    public function send_pm($sender_id, $receiver_id, $title, $message) {
        ...
        if ($this->config_vars['pm']['pm_encryption']) {
        ...
        return $this->aauth_db->insert($this->config_vars['table']['pms'], $data);
    }
    ...
    public function send_pms($sender_id, $receiver_ids, $title, $message) {
        ...
        foreach ($receiver_ids as $receiver_id) {
            ...
            $return_array[$receiver_id] = $this->aauth_db->insert($this->config_vars['table']['pms'], $data);
        }
    ...
    public function list_pms($limit = 5, $offset = 0, $receiver_id = NULL, $sender_id = NULL) {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['pms'], $limit, $offset);
    ...
    public function get_pm($pm_id, $user_id = NULL, $set_as_read = TRUE) {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['pms']);
    ...
    public function delete_pm($pm_id, $user_id = NULL) {
        ...
        if ($user_id == $result->sender_id) {
            if ($result->pm_deleted_receiver == 1) {
                return $this->aauth_db->delete($this->config_vars['table']['pms'], array('id' => $pm_id));
            }

            return $this->aauth_db->update($this->config_vars['table']['pms'], array('pm_deleted_sender' => 1), array('id' => $pm_id));
        } else if ($user_id == $result->receiver_id) {
            if ($result->pm_deleted_sender == 1) {
                return $this->aauth_db->delete($this->config_vars['table']['pms'], array('id' => $pm_id));
            }

            return $this->aauth_db->update($this->config_vars['table']['pms'], array('pm_deleted_receiver' => 1, 'date_read' => date('Y-m-d H:i:s')), array('id' => $pm_id));
        }
    }
    ...
    public function cleanup_pms() {
        $pm_cleanup_max_age = $this->config_vars['pm']['pm_cleanup_max_age'];
        $date_sent = date('Y-m-d H:i:s', strtotime("now -" . $pm_cleanup_max_age));
        $this->aauth_db->where('date_sent <', $date_sent);

        return $this->aauth_db->delete($this->config_vars['table']['pms']);
    }
    ...
    public function set_as_read_pm($pm_id) {
        ...
        $this->aauth_db->update($this->config_vars['table']['pms'], $data, "id = $pm_id");
    }
     ...
     */
    public function print_errors($divider = '<br />', $return = false) {
        $msg = '';
    ...
    public function get_errors_html($divider = '<br />') {
        return $this->print_errors($divider, true);
    }
    ...
    public function set_user_var($key, $value, $user_id = FALSE) {
        ...
        if ($this->get_user_var($key, $user_id) === FALSE) {
            ...
            return $this->aauth_db->insert($this->config_vars['table']['user_variables'], $data);
        }
        // if var already set, overwrite
            ...
            return $this->aauth_db->update($this->config_vars['table']['user_variables'], $data);
        }
    }
    ...
    public function unset_user_var($key, $user_id = FALSE) {
        ...
        return $this->aauth_db->delete($this->config_vars['table']['user_variables']);
    }
    ...
    public function get_user_var($key, $user_id = FALSE) {
        ...
        $query = $this->aauth_db->get($this->config_vars['table']['user_variables']);
    ...
    public function generate_recaptcha_field() {
        ...
        if ($this->config_vars['login_control']['ddos_protection'] && $this->config_vars['login_control']['recaptcha_active'] && $this->get_login_attempts() >= $this->config_vars['login_control']['recaptcha_login_attempts']) {
            ...
            $siteKey = $this->config_vars['login_control']['recaptcha_siteKey'];
